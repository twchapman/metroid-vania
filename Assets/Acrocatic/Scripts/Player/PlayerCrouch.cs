...
public class PlayerCrouch : MonoBehaviour {
	...
	void Update () {
		...
		// If the player is not grounded, air sliding is allowed, not performing a dash, not on a ladder and the player is pressing the crouch button...
		if (!player.grounded && allowAirSlide && slideAllowed && !player.dashing && !player.onLadder && crouchSlide) {
			// ... perform the slide.
			Slide();
