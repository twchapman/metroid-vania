...
public class PlayerWall : MonoBehaviour {
	...
	void Update () {
		...
		if (((jumpToStick && !player.grounded) || (!jumpToStick && !player.crouching)) && (wallJump.enabled || wallRun.enabled || wallSlide.enabled) && !player.onLadder) {
			// The player is facing a wall if a linecast between the 'sideCheck' objects hits anything on the wall layer.
			Collider2D hitWall = Physics2D.OverlapArea(player.sideCheckTop.transform.position, player.sideCheckBot.transform.position, wallLayer);
